...
| Stale epochs have no architectural or memory side effects | `rtl/execute/architectural_writeback.sv`; memory commit points planned | ALU writeback plus epoch clear/relaunch differential tests | Exhaustive writeback stale-cancellation and side-effect proof | Stale cancellation and epoch-1 relaunch | Verified for pre-memory sources; memory extends this requirement |
...
| Kernel done requires all warps exited and complete machine drain | `rtl/core/simt_core.sv` | Four-warp, lifecycle, backpressure, cancellation, and recovery tests | Drain precondition assertions and detected early-done mutation | ALU/multiplier/writeback blockers exercised | Verified for pre-memory; memory blockers will extend the equation |

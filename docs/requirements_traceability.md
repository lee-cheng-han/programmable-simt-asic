...
|---|---|---|---|---|---|
| Completion commits exactly once or is explicitly cancelled | Completion fabric and elastic writeback queue | Directed queue tests plus `backpressure_differential_test` | Stable commit/counter handshakes; queue conservation formal pending | Completion source × execution/writeback stall | Partial: randomized dual-boundary stalls preserve exact model-matched commit counts |
...
| Four-warp architectural behavior agrees with an independent reference | `model/emulator/multi_warp_emulator.cpp`, `rtl/core/simt_core.sv` | Directed arithmetic/divergence plus directed, constrained-random, backpressure, and structured-control UVM differential tests | Unique epoch/warp/sequence trace keys; model/RTL first-mismatch comparator; UVM sequence-continuity scoreboard | Directed 24, random 60/48, stalled 24, clear/relaunch 48, structured 19/22/33/38/44 events | Partial: integer dependencies, shallow/nested control with 1–4 warps, and dual-boundary stalls agree; randomized recovery, mid-flight cancellation, and memory remain |

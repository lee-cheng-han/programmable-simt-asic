...
| Four-warp architectural behavior agrees with an independent reference | `model/emulator/multi_warp_emulator.cpp`, `rtl/core/simt_core.sv` | Four-warp arithmetic and simultaneous divergence regressions | Unique warp/sequence trace keys; model/RTL first-mismatch comparator | 24 arithmetic and 44 divergence events across all four warps | Partial: arithmetic, multiplier dependencies, WID, and divergence agree; lifecycle/epoch/memory remain |

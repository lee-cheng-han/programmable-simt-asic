...
| Four-warp architectural behavior agrees with an independent reference | `model/emulator/multi_warp_emulator.cpp`, `rtl/core/simt_core.sv` | Directed arithmetic/divergence plus directed, constrained-random, and backpressure UVM differential tests | Unique epoch/warp/sequence trace keys; model/RTL first-mismatch comparator; UVM sequence-continuity scoreboard | Directed 24, random 60/48, stalled 24, clear/relaunch 48, divergence 44 events | Partial: integer dependencies and dual-boundary stalls agree across two seeds; structured random control, mid-flight cancellation, and memory remain |
